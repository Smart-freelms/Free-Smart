...
export const QuizTaker: React.FC<QuizTakerProps> = ({ quiz, user, onComplete }) => {
  ...
  useEffect(() => {
    if (timeRemaining !== null && timeRemaining > 0 && timeRemaining % 5 === 0) {
      ...
      if (question.type === 'multiple-choice' || question.type === 'true-false') {
        if (Array.isArray(question.correctAnswer)) {
          isCorrect = Array.isArray(userAnswer) &&
                      ...
                      userAnswer.every(val => (question.correctAnswer as string[]).includes(val));
        } else {
          isCorrect = userAnswer === question.correctAnswer;
        }
      } else {
        // For text-based answers, do a case-insensitive comparison
        const correctAnswer = (Array.isArray(question.correctAnswer) ? question.correctAnswer[0] : question.correctAnswer as string).toLowerCase().trim();
        const userAnswerText = (Array.isArray(userAnswer) ? userAnswer[0] : userAnswer as string || '').toLowerCase().trim();
