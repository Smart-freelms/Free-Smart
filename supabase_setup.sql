-- ############################################################################
-- SUPABASE DATABASE SETUP SCRIPT FOR SMART LMS (VERSION 2.5)
-- Final fix for RLS 403 Forbidden and UUID 400 Bad Request errors.
-- Optimized with explicit INSERT policies and aliases to prevent 403 errors.
-- ############################################################################
...
CREATE POLICY "attempts_update" ON public.quiz_attempts FOR UPDATE USING (auth.uid() = user_id OR EXISTS (SELECT 1 FROM public.quizzes q WHERE q.id = quiz_attempts.quiz_id AND q.created_by = auth.uid()));
...
CREATE POLICY "submissions_update" ON public.submissions FOR UPDATE USING (auth.uid() = student_id OR EXISTS (SELECT 1 FROM public.assignments a WHERE a.id = submissions.assignment_id AND a.created_by = auth.uid()));

-- 4.7 NOTIFICATIONS POLICIES
CREATE POLICY "notifications_select" ON public.notifications FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "notifications_insert" ON public.notifications FOR INSERT WITH CHECK (true); -- Permit creation by system/teachers
CREATE POLICY "notifications_update" ON public.notifications FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "notifications_delete" ON public.notifications FOR DELETE USING (auth.uid() = user_id);

-- 4.8 NOTIFICATION SETTINGS POLICIES
CREATE POLICY "settings_select" ON public.notification_settings FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "settings_insert" ON public.notification_settings FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "settings_update" ON public.notification_settings FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "events_select" ON public.scheduled_events FOR SELECT USING (auth.uid() = created_by);
CREATE POLICY "events_insert" ON public.scheduled_events FOR INSERT WITH CHECK (auth.uid() = created_by);
CREATE POLICY "events_update" ON public.scheduled_events FOR UPDATE USING (auth.uid() = created_by);
CREATE POLICY "events_delete" ON public.scheduled_events FOR DELETE USING (auth.uid() = created_by);
